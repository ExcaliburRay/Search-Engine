...
public class QryParser {
    ...
    public static List<Double> weightList= new ArrayList<>();
    ...
    private static Qry createOperator(String operatorName) {
        ...
        //detect the near operator
        //check the length of operator
        if(operatorNameLowerCase.length() >= 6) {
            //check the value and type of operator
            int size = operatorNameLowerCase.length();
            if (operatorNameLowerCase.substring(0, 5).equals("#near")) {
                String dist = operatorNameLowerCase.substring(6,size);
                operator = new QryIopNear(dist);
            }
            if(operatorNameLowerCase.length() >= 8){
                if (operatorNameLowerCase.substring(0,7).equals("#window")){
                    String dist = operatorNameLowerCase.substring(8,size);
    ...
    private static Qry parseString(String queryString)
        ...
        while (queryString.length() > 0) {
            ...
            if (queryString.charAt(0) == '#') {    // Subquery
                ...
                qargs = new Qry[1];
                //System.out.println(p.getPopped()+" has #");
            ...
            for (int i = 0; i < qargs.length; i++) {
                ...
                if(queryTree instanceof QrySopWSum){
                    ((QrySopWSum)queryTree).AddIntoWeightList(weight);
                }
                if(queryTree instanceof QrySopWAnd){
                    ((QrySopWAnd)queryTree).AddIntoWeightList(weight);
                }
    ...
    //test one string belong to double or not
